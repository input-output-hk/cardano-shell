...
First of all, we need to understand that the blocks in the blockchain contain the version of Daedalus (the frontend).
We can imagine that each block can contain a version of the frontend, which is essentially a hash signature from the installer.
...
> frontendVersion :: Block -> FrontendVersion
> frontendVersion block = SET (block |-> frontendVersion) ()
